...
public function index(Request $request)
{
    ...
    // WR-3 dan Keuangan hanya bisa melihat user dengan role 'user' (pendaftar)
    ...
    // Filter by role (kecuali untuk wr-3 dan keuangan yang sudah difilter di atas)
    if ($request->filled('role') && $currentUserRole !== 'wr-3' && $currentUserRole !== 'keuangan') {
    ...
    // ========== SORTING LOGIC BERDASARKAN ROLE ==========
    if ($currentUserRole === 'wr-3') {
        // WR-3: Urutkan yang BELUM verifikasi wawancara di atas
        $query->orderBy('is_wawancara_selesai', 'asc');
        $query->orderBy('created_at', 'desc');
        
    } elseif ($currentUserRole === 'keuangan') {
        // Keuangan: Urutkan yang BELUM bayar pendaftaran dan BELUM bayar daftar ulang di atas
        $query->orderByRaw('
            CASE 
                WHEN is_bayar_pendaftaran = 0 AND is_ukt_paid = 0 THEN 1
                WHEN is_bayar_pendaftaran = 0 AND is_ukt_paid = 1 THEN 2
                WHEN is_bayar_pendaftaran = 1 AND is_ukt_paid = 0 THEN 3
                WHEN is_bayar_pendaftaran = 1 AND is_ukt_paid = 1 THEN 4
                ELSE 5
            END ASC
        ');
        $query->orderBy('created_at', 'desc');
        ...
        // Role lain: urutkan berdasarkan created_at terbaru
        $query->orderBy('created_at', 'desc');
    }

    // Hitung total verified untuk statistik
    ...
    public function update(Request $request, string $id)
{
    ...
    /**
 * Verify interview for WR-3 role
 */
public function verifyInterview($id)
{
    $user = User::findOrFail($id);
    
    // Toggle status wawancara
    $user->is_wawancara_selesai = !$user->is_wawancara_selesai;
    $user->save();
    
    $status = $user->is_wawancara_selesai ? 'Terverifikasi' : 'Batal Verifikasi';
    
    return redirect()->route('admin.user.index')
        ->with('success', "Status wawancara untuk {$user->nama_lengkap} berhasil diubah menjadi {$status}.");
}

/**
 * Verify payment for Keuangan role
 */
public function verifyPayment(Request $request, $id)
{
    $user = User::findOrFail($id);
    $paymentType = $request->payment_type; // 'pendaftaran' or 'ukt'
    
    if ($paymentType === 'pendaftaran') {
        $user->is_bayar_pendaftaran = !$user->is_bayar_pendaftaran;
        $user->save();
        ...
        if ($user->is_bayar_pendaftaran) {
            if (!$payment) {
                ...
                \App\Models\Payment::create([
                    'user_id' => $user->id,
                    'order_id' => 'ADMIN-PENDAFTARAN-' . $user->id . '-' . time(),
                    'jumlah' => 0, // Sesuaikan dengan biaya yang berlaku
            ...
            if ($payment) {
                $payment->update(['status_transaksi' => 'pending-offline']);
            }
        }
        
        $status = $user->is_bayar_pendaftaran ? 'Terverifikasi' : 'Batal Verifikasi';
        return redirect()->route('admin.user.index')
            ->with('success', "Pembayaran pendaftaran untuk {$user->nama_lengkap} berhasil diubah menjadi {$status}.");
            
    } elseif ($paymentType === 'ukt') {
        $user->is_ukt_paid = !$user->is_ukt_paid;
        $user->save();
        ...
        $status = $user->is_ukt_paid ? 'Terverifikasi' : 'Batal Verifikasi';
        return redirect()->route('admin.user.index')
            ->with('success', "Pembayaran daftar ulang untuk {$user->nama_lengkap} berhasil diubah menjadi {$status}.");
    }
    
    return redirect()->route('admin.user.index')
        ->with('error', 'Tipe pembayaran tidak valid.');
}
}