...
class ProgramStudiSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Ambil mapping nama_fakultas -> id dari tabel fakultas
        $fakultasMap = DB::table('fakultas')->pluck('id', 'nama_fakultas');
        ...
        foreach ($prodis as $p) {
            $fakultasId = $fakultasMap[$p[3]] ?? null;

            if (is_null($fakultasId)) {
                $this->command->warn("Fakultas tidak ditemukan: {$p[3]} (prodi: {$p[1]}) — dilewati.");
                continue;
            }
        ...
        $this->command->info('Program studi berhasil diseeder.');
    }
}