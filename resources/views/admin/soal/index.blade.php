        ...
        <div class="hidden lg:block overflow-x-auto">
            ...
            <div class="px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-gray-100">
                ...
                <p class="text-sm text-gray-600 order-2 sm:order-1">
                    Menampilkan
                    <span class="font-semibold text-blue-700">{{ $soals->firstItem() ?? 0 }}</span>
                    –
                    <span class="font-semibold text-blue-700">{{ $soals->lastItem() ?? 0 }}</span>
                    dari
                    <span class="font-semibold text-blue-700">{{ $soals->total() }}</span>
                    data
                </p>
                ...
                @if ($soals->hasPages())
                    <div class="flex items-center gap-1 flex-wrap justify-center order-1 sm:order-2">
                        ...
                        @else
                            <a href="{{ $soals->previousPageUrl() }}" class="px-3 py-2 rounded-lg text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all duration-200 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                                <span class="hidden sm:inline">Prev</span>
                            </a>
                        @endif
                        ...
                        @if ($soals->hasMorePages())
                            <a href="{{ $soals->nextPageUrl() }}" class="px-3 py-2 rounded-lg text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all duration-200 flex items-center gap-1">
